...
\title{Active learning speeds acquisition of a Zipf-distributed vocabulary from limited data} 
...
\keyword{Keywords: Word learning; Active learning; Zipf’s law; Fast mapping; Linear programming; Word frequency}

\authornote{The authors have no conflict of interest to declare.}

% abstract is XXX words
\abstract{
Children learn thousands of words in the first several years of life, which has inspired decades of research into the mechanisms of early word learning. 
The present study revisits past theoretical analyses of how children may learn a full-sized vocabulary, considering more realistic Zipfian (i.e., power-law) distribution of words and referents drawn from a corpus of child-directed speech. 
We offer new analytical bounds on the performance of past models, as well as generalized results for arbitrary frequency distributions.
We demonstrate that the simulated speed of word learning from naturalistic data is somewhat slower than expected from past work.
In light of recent empirical work showing that learners actively select their learning environment, we propose and analyze a variety of active learning mechanisms. 
We show both via simulation and mathematical analysis that active learners can learn orders of magnitude faster than passive learners given randomly-sampled situations. 
Based on these analyses, we suggest which characteristics of children's real-world learning may be diagnostic of their engagement in active learning.}
...
By 18 months, children can produce on average 50 words and comprehend 100-150 \citep{Hulit:2002}, with productive vocabulary size growing to around 1,000 words by 36 months \citep{Hart:1995}, and 2,100-2,200 words by age 5 \citep{Lorraine2008}, when word learning further accelerates in school.
Estimates for the average vocabulary size of college-aged adults range from 20,000 word families [CITE] to roughly 60,000 words \citep{Bloom:2000}.
%Under the assumption that each child has 9 hours of word learning opportunity every day, these estimates have the child learning a new word every learning hour for 18 years, on average. 
% The average child learns 6-7 words per day from ages 6 to 8, and approximately 12 words per day from ages 8 to 10 (Bloom & Markson, 1998)
...
Although older learners no doubt accelerate word learning as they begin to read, children already show an accelerating pace of learning in the second year of life, growing from an average of 11 words at 12 months to over 300 words by 24 months \citep{wordbankbook,fenson2007}.
How might young children tackle the daunting problem of mapping words to referents?

Researchers have proposed a variety of heuristics and mechanisms that learners may leverage to help them mitigate referential uncertainty, and empirical studies have shown that young children can take advantage of many of them--at least in the laboratory. 
For example, children can leverage social cues \citep{HirshPasek:2000}, can learn a word from a single informative instance \citep[i.e., {\em fast-map};][]{Carey:1978,Spiegel:2011}, tend to assume words refer to whole objects \citep{Macnamara:1972}, and are biased to form mutually exclusive word-referent mappings \citep{Merriman:1989}. 
These heuristics are assumed to reduce the complexity of the learner's problem of {\it cross-situational learning}: acquiring words by accumulating evidence for meanings across multiple contexts--an ability demonstrated in the lab by infants and adults \citep{Smith:2008vg,Yu:2007}. 
However, determining the relative contributions of these myriad heuristics to naturalistic cross-situational word learning remains a challenge, despite their inclusion in various computational models of word learning, both because such models are primarily applied to experimental settings, and because the complexity and structure of real-world learning environments is largely unknown \citep[though see][]{Smith:2018,Suanda:2018,Clerkin:2017}.
...
Past theoretical analyses of word learning have largely focused on the learning time of vocabularies with words randomly sampled from various frequency distributions (e.g., Zipfian or uniform), without considering the active role that learners (or beneficent caregivers) may take in selectively shaping the learner's input.

\citep{gottlieb2018towards}

\subsection{Active learning}
Various active learning algorithms have been analyzed in machine learning, where it has been found time and again to be faster (i.e., require fewer training instances) than learning from randomly-selected instances \citep{Settles:2012}.\footnote{It is worth noting that active learning can also lead to undesirable bias if the implemented algorithm makes incorrect assumptions about the distribution. But in general, for well-understood problems there must exist an active learning algorithm that is more sample-efficient than a }

Recently, studies of human word learning have found that adults show superior peroformance when given active control of their word learning \citep{Kachergis:2013, zettersten2021sampling}.
Further, children (ages X-Y) have also been shown to be capable of learning (with similar performance?) \citep{zettersten2021sampling}.

\citep{clerkin2019everyday}

As a starting point, we build on past mathematical analyses of word learning, which assume learners passively experience a series of situations, each consisting of a random sample of a single word, a target meaning, and several distractor meanings. 
