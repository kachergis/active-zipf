...
Using both mathematical analysis and simulation--including a more realistic incremental learner that does not assume words are learned in a single exposure--we show that such active learners acquire Zipfian vocabularies substantially faster than passive learners exposed to the same random samples, with the size of the advantage depending on how idealized the learning scenario is.
...
Throughout this paper we follow prior theoretical work in analyzing the feasibility of an 18-year, 60,000-word target \citep{Blythe:2010,Vogt:2012,Blythe:2016}, since this is the estimate against which our results are most directly comparable.
It is worth being explicit, however, about which portion of vocabulary growth this target best describes.
A commonly-cited developmental trajectory has children recognizing about 50 words at age one, about 1,000 words by age three, and at least 10,000 words by age five \citep{Shipley:2015}--consistent with independent estimates of roughly 10,000 root-word meanings known by early elementary school \citep{Anglin:1993}.
Age five is also, for most children, when formal reading instruction begins, after which written text becomes an increasingly important source of new vocabulary, one governed by a different frequency distribution, a different sampling process, and plausibly different active and passive mechanisms than the largely auditory, caregiver-mediated input we model here.
The sampling and learning models we analyze--which assume a single, stationary word frequency distribution encountered through spoken situations--are therefore most directly applicable to this pre-literacy period, during which vocabulary growth is driven overwhelmingly by spoken input rather than text, and less directly applicable to the school-age years during which the bulk of the 60,000-word target is actually reached.
We are explicit about this scope limitation up front because it shapes how the 60,000-word, 18-year figures used throughout the paper (following past work) should be read: as a common point of comparison with prior theoretical analyses, not as a claim that our spoken-language sampling models are meant to describe word learning across the full 18-year span.
We return to the implications of this scoping for our conclusions in the Discussion.
...
A caveat about this estimate is worth making explicit, since Equation~(\ref{eq-Tbound})'s bound $T_{+}$ is driven entirely by the single least-frequent word in the corpus.
This frequency distribution is aggregated across 4,163 separate transcripts, each contributed by a different family; the very lowest-frequency words in Figure~\ref{fig-freq} may therefore include words idiosyncratic to a single household (e.g., a pet's or sibling's name) rather than words that are genuinely rare in any individual child's actual input.
If so, the corpus-wide minimum overstates how rare the rarest word in an individual child's own environment really is, and our $T_{+}$ estimate would overstate that child's learning time.
...
\]
In words, $\hat q$ is whatever mixture of situations makes the \textit{least}-likely word-referent pair as likely as possible--maximizing the minimum entry of $Pq$--because it is exactly this minimum probability that, per Section~\ref{sec-fastmap}, determines how long the learner must wait to complete the vocabulary. Framed this way, active learning as formalized here does not change how quickly any individual word is learned once encountered; rather, it reshapes which words the learner is likely to encounter in the first place, spreading exposure more evenly across the vocabulary than random sampling of situations would.
...
This tail comparison also clarifies an important limitation of the comparison: the word-referent distribution realized under uniform scene selection in this dataset is not itself Zipfian, and its exponential tail is in fact \textit{less} heavy--and thus easier to learn from--than the Zipfian distribution assumed as the passive baseline in Section~\ref{sec-fastmap}. The baseline passive learner analyzed here is therefore the passive learner implied by this particular scene database under uniform scene selection, not a learner facing the specific Zipfian word frequency distribution analyzed in Part 1 of this paper; the two are related but not identical comparisons. If anything, this suggests the $235\times$ estimate is conservative relative to a passive learner constrained to face a genuinely Zipfian marginal distribution matched to Section~\ref{sec-fastmap}'s assumptions, since the exponential-tailed baseline realized here should already be somewhat easier to learn from than a Zipfian one. Constructing $P$ so that its uniform-$q$ marginal is constrained to exactly reproduce a specified Zipfian distribution--allowing a matched, apples-to-apples comparison with Part 1--is a natural target for future work with richer scene databases.
...
The analyses above--both the fast-mapping bound of Section~\ref{sec-fastmap} and the idealized and online active learners of Section~\ref{sec-active}--all assume that a word is learned the moment it co-occurs with its target referent. This lets us derive clean analytical results, but it sidesteps a concern raised about this modeling tradition more broadly: that conclusions about learning speed may reflect the {\it sampling} process (how often the right word-referent pair happens to co-occur) rather than anything about {\it learning} itself, since treating learning as instantaneous leaves no room for the two to be distinguished. To test whether our qualitative conclusions--that passive sampling from a Zipfian distribution is implausibly slow, and that active learners can close much of this gap--survive when this instantaneous-learning idealization is relaxed, we simulated a learner who accumulates evidence across multiple exposures rather than fast-mapping. This simulation leaves a second idealization untouched: as in all the models in this paper, words are still learned fully independently of one another, with no transfer of information from an already-learned word (e.g., ``dog'') to a related but distinct one (e.g., ``cat''). We return to this remaining simplification, and its likely direction of bias, in the Discussion.

\subsection{An incremental, mutual-exclusivity learner}
Following \cite{Vogt:2012} and \cite{Siskind:1996}, we implemented a learner who tracks, for each word, the set of referents it remains consistent with across all past exposures. In each episode, the learner observes a target word alongside $C-1$ distractor referents (as in Section~\ref{sec-active}); rather than immediately committing to a mapping, the learner eliminates from the word's candidate set any referent that was absent from the current situation. A word is considered learned once its candidate set has been narrowed to a single referent--i.e., once enough situations have ruled out every other possibility. This is a strictly harder learning problem than fast-mapping: a single co-occurrence is no longer sufficient, and how quickly a word is learned now depends on {\it which} distractors happen to accompany it, not only on how often it is sampled.
...
We simulated vocabularies of $M = 1000$ and $M = 2000$ referents, with Zipfian word frequencies following the Zipf-Mandelbrot form $p_r \propto (r + 2.7)^{-1}$ over frequency rank $r$ \citep{Mandelbrot:1953}. Context sizes $C$ ranged from 5 to 200, always satisfying $C \le 0.1M$ per \cite{Vogt:2012}'s finding that learning becomes intractable outside this range. For each combination of $M$, $C$, target selection (active/passive), context selection (random/familiar), and word frequency distribution (uniform/Zipfian), we ran 15--100 replications and recorded the number of episodes needed to learn 99\% of the vocabulary.\footnote{Simulation code and raw results are available in the project repository (\texttt{corpusXSL\_eliminative\_active\_parallel.R}, \texttt{activeXSL\_simulations.Rmd}).}

\subsection{Results}
Figure~\ref{fig-incremental-sim} shows the mean number of episodes needed to learn 99\% of a $M=1000$-word vocabulary, for the subset of context sizes ($C=10, 100$) simulated under all four combinations of target and context selection. Under a uniform WFD, active target selection alone yields a modest speedup over passive sampling (2,894 vs.\ 8,340 episodes, a 2.9x reduction), and familiar-context selection adds essentially nothing further--unsurprising, since eliminating distractors is not especially hard when every referent is equally frequent to begin with.

Under a Zipfian WFD, the pattern is very different, and directly addresses the concern that our earlier fast-mapping results might not generalize to a learner that must accumulate evidence over time. Fully passive learning (random target and context selection) requires an average of 5,893,267 episodes--consistent with the implausibly slow learning implied by the fast-mapping analysis of Section~\ref{sec-fastmap}, now obtained from a learner that never fast-maps at all. Active target selection alone cuts this to 357,958 episodes, a 16.5x speedup, comparable in order of magnitude to the online active learner of Section~\ref{sec-active}. Familiar-context selection alone (with passive target selection) is even more effective, reducing learning time to 56,232 episodes (a 104.8x speedup)--because once a few referents have been narrowed down, using them as distractors makes every subsequent elimination nearly free. Combining active target selection with familiar-context selection reduces learning time to 5,809 episodes on average, barely above the $M=1000$ lower bound of one episode per word, and over 1,000 times faster than fully passive learning. In other words, once the learner is allowed to shape not only {\it which} word they hear next, but also {\it what else is in the scene} when they hear it, an otherwise implausibly slow Zipfian learning problem becomes nearly as fast as the uniform case.

\begin{figure}[h]
\begin{center}
\includegraphics[width=\linewidth]{incremental_sim_summary.pdf}
\caption{\label{fig-incremental-sim}
Mean episodes (log scale, $\pm$ 1 SE) required for an incremental, elimination-based learner to acquire 99\% of a 1000-word vocabulary, as a function of whether the target word is sampled passively or actively selected to be unknown (bar fill), and whether distractors are drawn randomly or preferentially from familiar (already partly-known) referents (x-axis), separately for uniform and Zipfian word frequency distributions. Results are averaged over context sizes $C=10$ and $C=100$.}
\end{center}
\end{figure}

These results reinforce the paper's central conclusions using a learner that makes neither of the two idealizations most often raised against fast-mapping models: words are not learned on a single exposure, and the number of samples required is not simply a proxy for how often a word is spoken. They also reveal that the two forms of active control we distinguish--choosing what to talk about, and choosing what else is nearby when talking about it--are not interchangeable: under a Zipfian distribution, controlling the distractor context turns out to matter at least as much as controlling the target word itself, a distinction that the idealized analysis of Section~\ref{sec-active} does not draw out, since it collapses both into a single choice of situation $j$.
...
Our new analysis showed that even fast-mapping--the most generous passive, independent learning scheme--implies an implausibly large number of samples for word distributions with a long tail of rarely-sampled words, including the Zipf distribution, and simulations of a more realistic incremental learner showed the same qualitative pattern (Section~\ref{sec-incremental-sim}). % In essence, learners end up waiting a long time for the long tail of the distribution: low-frequency words are simply too rare.
...
Analyzing an empirical dataset of labeled referents in a set of natural scenes, we found that the expected learning time of an idealized active learner--one with full advance knowledge of how referents co-occur with scenes, who still fast-maps each word--is over two hundred times (235x) more efficient than a passive learner sampling scenes uniformly at random.
Two further analyses show that this large speedup is best treated as an upper bound rather than a representative estimate, for two largely independent reasons.
First, an active learner who must instead estimate this co-occurrence structure online from experience, rather than know it in advance, but who still fast-maps, requires only about half as many samples as its passive counterpart in our simulated setting (Section~\ref{sec-active})--a much smaller advantage than the idealized case, since the learner spends part of its budget discovering $P$ rather than exploiting it.
Second, and separately, a learner who cannot fast-map at all, but must instead accumulate evidence for word-referent mappings across multiple exposures, shows a roughly 16-fold advantage for active target selection alone under a Zipfian distribution (Section~\ref{sec-incremental-sim})--and a more than 1,000-fold advantage once the learner can also shape which distractors accompany the target word, rather than only which word is targeted.
Taken together, these estimates span roughly two orders of magnitude, from about 2x to over 1,000x, depending on which idealizations are relaxed: how much the learner already knows about its environment (a known vs.\ estimated $P$), whether learning requires one exposure or several, and whether active control extends only to the target word or also to its surrounding context.
What remains stable across this range is the direction and rough scale of the effect, not its exact size: some form of active control over learning situations meaningfully speeds up vocabulary acquisition under Zipfian word and referent distributions, and does so far more than under a uniform distribution, where active selection helps only modestly (Section~\ref{sec-incremental-sim}).
...
We do not think this null result undermines our central contribution--which is a computational characterization of how large an advantage active control {\it could} confer under Zipfian distributions, not a claim that children realize this advantage in full--but it is a reason for caution in treating our active-learning estimates as a description of typical development, rather than as an upper bound that motivates looking for partial, more constrained forms of active control.
