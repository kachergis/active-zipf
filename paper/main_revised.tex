...
Using both mathematical analysis and simulation--including more realistic incremental learners that do not assume words are learned in a single exposure--we show that such active learners acquire Zipfian vocabularies substantially faster than passive learners exposed to the same random samples, with the size of the advantage depending on how idealized the learning scenario is, and growing severalfold between a functional and a fully complete vocabulary.
This advantage does not depend on an idealization of perfect, unlimited memory for tentative word-referent guesses: when unrehearsed guesses can be forgotten before they are confirmed, active learning slows modestly while passive learning slows by orders of magnitude, so the advantage widens sharply as memory becomes less durable.
...
\item {\bf The advantage survives, and strengthens under, forgetting.} We test whether this advantage depends on an idealization of perfect, unlimited memory for tentative guesses, and find that it does not merely survive a more cognitively realistic forgetting mechanism, but strengthens under it: when a guess must be confirmed before it sticks, active selection's habit of returning to not-yet-learned words doubles as rehearsal, which passive sampling of a Zipfian vocabulary cannot provide for its rare words.
\end{itemize}
...
\end{equation}
For an adult-sized vocabulary $W = 60,000$, the expected number of samples is $E[T] = 694,759$ (the approximation $W \log W$ gives 660,126).
...
Thus, instead of the exact expression of $T$, we consider the upper and lower bound for $T$ (see Appendix~\ref{app-bound} for the derivation).
...
Compared on the same criterion ($\epsilon = 0.01$), this is 11.6 times as many samples as a uniform distribution of the same size requires ($T \approx 936{,}000$; Equation~\ref{eq-T}), in close agreement with the factor of 11.58 that \cite{Blythe:2010} report for this case. The bound is also tight here: solving Equation~(\ref{eqA-M}) exactly gives $T = 8.97 \times 10^{6}$, only 1.2 times smaller than $T_{+}$.
...
%where $H_{W,a}$ is the generalized harmonic number $H_{W,a} = \sum_{i=1}^{W}i^{-a}$.
With some calculation (see Appendix~\ref{app-zipf}), we find the upper bound $T_{+}$ (Equation \ref{eq-Tbound}) as a function of the exponent parameter $a$ holds the bound
\[
...
The required number of samples ($T$) from a generalized Zipf distribution ($p_{k} = k^{-a}/\sum_{k=1}^{W}k^{-a}$) needed for 1\%, 50\%, and 99\% of learners ($\epsilon$ = [.99, .01]; dotted lines, $\epsilon$ = 0.5; solid line) to reach a vocabulary size of $W = 10000$ or $W = 60000$, for varying values of the distribution exponent $a = 0, \ldots, 1.5$. A value of $a = 0$ corresponds to a uniform distribution. The number of samples is numerically calculated by the root of Equation (\ref{eqA-M}) in Appendix~\ref{app-bound}.} % what's the empirical a for CHILDES? ~1.5?
\label{fig}
\end{center}
\end{figure}
...
Denote the $N \times M$ matrix of conditional probabilities by $P = \{ p_{ij} \}_{ij}$ and the $M \times 1$ vector of the situation choice probabilities by $q = (q_{1}, q_{2}, \ldots, q_{M})^{T}$.
...
This minimal probability, $\min(P\hat{q})$, gives the theoretical upper bound for the minimal number of samples $$T_{+}=f_{N,\epsilon}( \min(P \hat{q}) ).$$
This is an upper bound with the known $P$, because the probability matrix
$P$ is not known before empirical learning, and the active learner also needs to estimate $P$ from the sample.

For a given matrix $P$, the optimal $\hat{q}$ can be computed by the iterated linear programming algorithm (see Appendix~\ref{app-ilp} for details). % algorithmic complexity of this? Iris van Rooij and Johan Kwisthout would care

As a baseline for the passive learner, we take uniform scene choice, $q = \one{M}/M$--a learner (or caregiver) who constructs scenes without regard to which words the learner has already mapped to a referent, the counterpart of the randomly-sampled situations analyzed in the previous section, now expressed in terms of situations rather than words directly. This baseline is, if anything, generous to the passive learner: because $\min(Pq)$ is a concave function of $q$, Jensen's inequality gives $$\int_{q\in \Simplex{M}}\min( Pq ) (M-1)!\dd q \le \min( P \one{M}/M ),$$
where the integral is taken over the $M-1$ dimensional unit simplex $q\in \Simplex{M}$, so a learner whose scene-choice probabilities are themselves drawn at random fares no better, on average, than one who chooses scenes uniformly.
For a sufficiently small $x \ll 1$ and $y \ll 1$, $f_{N,\epsilon}(x)/ f_{N,\epsilon}(y) \approx y/ x$.
Thus, the rate $$R = \min(P \hat{q})/ \min(P \one{M}/M)$$ gives a good estimate of how many times faster active learning with the optimal probability $\hat{q}$ is than passive learning with uniform scene choice, and a conservative one relative to a randomly chosen $q$.
...
The underlying data also let us ask how the passive marginal realized under uniform scene selection compares with the idealized Zipfian distributions analyzed in the Independent fast-mapping learning section.
...
For comparison, a standard idealized Zipfian distribution ($a=1$) over the same $N=3,458$ items would require only $T_{+}\approx 3.85\times10^{5}$ episodes under passive fast-mapping. The real, empirically-derived passive object distribution is therefore roughly 3,990 times harder to learn from than an idealized Zipfian of the same size, and even the optimal active learner ($6.52\times10^{6}$ episodes) remains about 17 times slower than a passive learner facing that idealized distribution. Real-world object distributions realized by unstructured (uniform) scene sampling are thus considerably more extreme than the textbook Zipfian case, and active selection, although it recovers a large share of the resulting difficulty, does not remove it.
...
A second, independent real dataset gives a qualitatively consistent but quantitatively smaller result. Repeating this analysis on Flickr8k \citep{Hodosh:2013}--8,108 photographs, each independently described by five human captions, with words in place of objects and photos in place of scenes--gives $\min(Pq) = 2.68\times10^{-6}$ under uniform photo selection and $\min(P\hat q) = 1.80\times10^{-5}$ under the optimal $\hat q$, a $6.7\times$ speedup. The real passive marginal is again harder to learn from than an idealized Zipfian distribution of the same size ($N=4{,}438$ words: $9.4\times$ harder, vs.\ SUN's $3{,}990\times$)--the same direction of effect as SUN, but far more modest in magnitude. We attribute this difference to a structural feature of the two datasets rather than to anything specific to language versus vision: SUN's ``scenes'' are categories that pool object statistics across many images, which concentrates the resulting distribution considerably, whereas Flickr8k's ``situations'' are individual photographs described by only five captions each--a much less aggregated unit, leaving less room for an optimal $\hat q$ to exploit. Two independently-constructed real datasets nonetheless agree on the qualitative conclusion: passive random selection is harder than an idealized Zipfian baseline predicts, and active selection recovers a substantial share of the resulting gap.
...
Thus, we perform a Monte Carlo simulation to quantify the efficiency of an {\it online} active learner who gradually learns $P$ and estimates $q$ based on the sample estimate of $P$. 
If this online active learner performs comparably to the optimal active learner with $\hat{q}$, the optimal learner's performance is a reasonable guide to what a learner who must estimate $P$ could achieve--at least for matrices like the one simulated here. 
...
The online active learner starts with uniform choice probability $q_{1} = \one{M}/M$. 
...
Beyond the case study reported above, we also provide a theoretical analysis of why such online learning is possible in Appendix~\ref{app-online}.
...
\subsection{Incremental learning mechanisms}
We implemented two main learners that both require multiple exposures to learn a word--unlike fast-mapping--but that differ in how they use those exposures, letting us ask whether our conclusions are a property of incremental learning in general or an artifact of one particular mechanism. (A third, which tracks co-occurrence counts, is examined under Robustness across Zipf exponent and learning mechanism.)
...
The eliminative learner's per-episode cost in our reference implementation grows with both $M$ and $C$, which made its $C=100$ cells impractical to simulate under a steep Zipfian distribution. We therefore ran every eliminative $C=100$ cell with an exact reimplementation of the same learner (same context construction, same learning criterion, more efficient bookkeeping), which we validated against the reference implementation on all cells both could complete: mean episodes agreed within sampling error in every case ($|z| \le 1.03$ on the largest comparison). Runs were capped at $5 \times 10^8$ episodes, and runs that reached this cap are reported as such rather than dropped. We also do not report an $M=10{,}000$ extension of this design (which we attempted, motivated by the pre-literacy vocabulary target discussed in the Introduction): even active-target conditions at this scale proved unpredictably expensive for the eliminative learner, and we judged further compute expenditure not to be a good use of the resources available for this project. The guess-test learner's cost does not scale with $M$ in the same way and was tractable throughout.\footnote{Simulation code and raw results are available in the project repository (\texttt{corpusXSL/analysis/learners.R}; the fast eliminative learner and its validation are \texttt{learners\_fast.cpp} and \texttt{validate\_elim\_fast.R}). An earlier version of this analysis filled the eliminative $C=100$ cells with older simulations under a flatter Zipf--Mandelbrot distribution, $p_r \propto (r+2.7)^{-1}$; those numbers are superseded by the ones reported here.}

\subsection{Results: context size interacts with active control}
Table~\ref{tab-incremental} and Figure~\ref{fig-incremental-sim} summarize the eliminative learner's mean episodes to learn 99\% of a $M=1000$-word Zipfian ($a=1$) vocabulary, at $C=10$ and $C=100$. Two things stand out. First, replicating our earlier conclusion, passive random sampling is dramatically slower under a Zipfian than a uniform distribution, and active target and context selection both help. Second, and not previously appreciated, {\it which} form of active control matters depends sharply on $C$, in a way that connects directly to \cite{Vogt:2012}'s observation that learning time depends on the ratio $C/M$. At $C=10$ ($C/M=0.01$, well inside Vogt's tractable range), active target selection does most of the work (19.1x), and familiar-context selection adds only a modest further benefit (2.0x alone, 28.6x combined), because elimination is already fairly effective when the distractor set is small.

At $C=100$ ($C/M=0.1$, Vogt's reported boundary of tractability), random-context learning does not merely slow down under a Zipfian distribution: it fails. No passive or active run with random distractors reached even half the vocabulary within $5 \times 10^8$ episodes. The reason is the eliminative learner's requirement that every competing referent be absent from at least one of a word's contexts. With 99 frequency-weighted distractors per context, the most frequent referent ($p \approx 0.13$ at $a=1$) is absent from any given context with probability only about $5 \times 10^{-10}$ (computed exactly\footnote{By the exponential-clock representation of weighted sampling without replacement: each referent $j$ receives an independent clock $E_j \sim \mathrm{Exp}(p_j)$, the context's distractors are the $C-1$ referents whose clocks ring first, and referent $r$ is absent iff at least $C-1$ other clocks ring before $E_r$. The resulting one-dimensional integral over a Poisson-binomial tail agrees with direct simulation wherever the probability is large enough to simulate; see \texttt{corpusXSL/analysis/c100\_absence\_probability.R}.}), so every other word would need on the order of $2 \times 10^9$ exposures merely to rule out that one competitor. Active target selection cannot help here, since it changes which word is the target but not which distractors accompany it. Familiar-context selection, which does change the distractors, restores learning (446,962 episodes), and once distractors are controlled, active target selection adds a further 28-fold advantage (15,819 episodes). At $C=100$, in other words, controlling what else is in view when a word is heard is not merely the dominant lever, as it might appear under a flatter distribution, but a precondition for learning at all.

This failure is a matter of degree of skew, not an all-or-none property of $C=100$. Sweeping the Zipf exponent at $C=100$ (Figure~\ref{fig-exponent}, right) shows passive random-context learning slowing steeply but continuously, from 9,904 episodes at $a=0$ (uniform) to 40,456 at $a=0.5$, 1.45 million at $a=0.7$, 47 million at $a=0.8$, and beyond our $10^8$-episode cap from $a=0.85$ on, tracking the exact probability that the most frequent referent is absent from a given context (0.17 at $a=0.5$, $2.6 \times 10^{-4}$ at $a=0.8$, $1.5 \times 10^{-6}$ at $a=0.9$). Active target selection helps throughout this range (e.g., 12x at $a=0.8$) but succumbs in turn (38 million episodes at $a=0.85$, beyond the cap at $a=0.9$). Only familiar-context selection keeps learning fast as skew grows (at $a=0.9$: 106,039 passive and 10,646 active episodes).

\begin{table}[h]
\centering
\caption{\label{tab-incremental} Mean episodes to learn 99\% of a Zipfian ($a=1$), $M=1000$-word vocabulary, eliminative learner, 30 replications per cell. Speedup is relative to Passive/Random at the same $C$. At $C=100$, no random-context run reached even 50\% of the vocabulary within the $5 \times 10^8$-episode cap, so speedups there are lower bounds.}
\begin{tabular}{lrrrr}
\hline
 & \multicolumn{2}{c}{$C=10$ ($C/M=0.01$)} & \multicolumn{2}{c}{$C=100$ ($C/M=0.1$)} \\
Condition & Episodes & Speedup & Episodes & Speedup \\
\hline
Passive / Random    & 89{,}454    & 1x      & $>5 \times 10^8$ & 1x \\
Active / Random     & 4{,}695     & 19.1x   & $>5 \times 10^8$ & -- \\
Passive / Familiar  & 44{,}607    & 2.0x    & 446{,}962        & $>$1{,}100x \\
Active / Familiar   & 3{,}127     & 28.6x   & 15{,}819         & $>$31{,}000x \\
\hline
\end{tabular}
\end{table}
...
On the first question, the answer is reassuring: mean and median are close at every checkpoint and condition we examined (ratio of mean to median between 0.99 and 1.09 throughout, and within 1.05 in all but the passive guess-test learner at $C=100$). Despite each individual word's own acquisition time being itself a highly skewed waiting-time process (see the Independent fast-mapping learning section), the total time to reach a given fraction of an $M$-word vocabulary averages over enough of these individually-skewed waits that the aggregate is close to symmetric. The means reported throughout this paper are therefore not artifacts of a long right tail; a typical simulated learner looks very much like the reported average.

On the second question, the completion target matters a great deal--not for whether active selection helps, but for how large the benefit appears to be. Table~\ref{tab-checkpoint} and Figure~\ref{fig-checkpoint} show the active/passive speedup at each checkpoint. In every condition, active selection already helps at 50\% completion--a typical run through half the vocabulary is 1.2--4.1x faster under active selection. But the size of this advantage grows as the target moves toward completeness, in every condition: for the eliminative learner it roughly doubles by 90\% and at least doubles again by 99\%, reaching 19x at $C=10$ and 28x at $C=100$ (with familiar context), where the last stretch from 90\% to 99\% alone multiplies the advantage more than seven-fold; for the guess-test learner the growth is more gradual (from 1.7x to 4.9x at $C=10$, and from 1.2x to 1.8x at $C=100$). The single 99\%-only estimates reported elsewhere in this paper are therefore close to the maximum size the effect takes across this trajectory, not a representative one--informative in its own right, but not interchangeable with a claim about how much active selection typically matters for everyday word learning.
...
\begin{table}[h]
\centering
\caption{\label{tab-checkpoint} Active/passive speedup (ratio of mean episodes, passive over active; Zipfian, $a=1$) at three vocabulary-completion checkpoints. Context selection is random except in the eliminative $C=100$ row, which uses familiar context because no random-context run completes at $C=100$ (Table~\ref{tab-incremental}). Median-based speedups (not shown) agree to within 4\% throughout, except for guess-test at $C=100$, where the 99\% speedup is 1.6x by medians. The guess-test $C=100$ row comes from a separate run of 30 replications per condition that recorded all three checkpoints.}
\begin{tabular}{lrrr}
\hline
...
Eliminative, $C=100$ (familiar) & 2.1x & 3.8x & 28.3x \\
Guess-test, $C=10$   & 1.7x & 3.0x & 4.9x  \\
Guess-test, $C=100$  & 1.2x & 1.4x & 1.8x  \\
\hline
\end{tabular}
\end{table}
...
Median episodes (log scale; band = interquartile range) to reach 50\%, 90\%, and 99\% of a Zipfian ($a=1$), $M=1000$-word vocabulary, active vs.\ passive target selection, by learning mechanism and context size. The $C=100$ row uses familiar context selection, since no random-context run completes at $C=100$. The active/passive gap widens steadily with the completion target rather than being a fixed multiplicative constant.}
\end{center}
\end{figure}
...
We tested this directly by adding a minimal forgetting mechanism to the guess-test learner (chosen because its propose-and-verify structure already tracks a natural target for decay: each word's current best guess). For forgetting to be able to matter at all, one further change is needed: in the guess-test learner as described above, a word counts as learned the instant its guess happens to be correct, so the only guesses left for forgetting to erase are wrong ones--and a wrong guess, being checked lazily only when its word is next exposed, would be discarded at that same moment by disconfirmation anyway.\footnote{We confirmed this directly: adding forgetting to the original, instantly-locking guess-test learner left both active and passive learning unchanged at every half-life tested below (passive: 37,642--38,774 episodes to 99\%; active: 6,963--8,166; speedup 4.6--5.4x throughout).} We therefore made a guess \textit{lock in} only after it has survived $k=2$ consecutive re-exposures of its word (i.e., been confirmed twice); until then, any guess--correct or not--decays back to ``no guess'' with a constant per-episode hazard if the word goes unrehearsed. Concretely, a guess survives $t$ episodes without its word being re-exposed with probability $(1-p)^t$, checked lazily only when the word is next targeted, and each confirmation counts as rehearsal and restarts this clock. Locked guesses are exempt: this is a claim about forgetting a tentative hypothesis, not about relearning already-acquired words, a different and more contentious claim we do not make here. A word is counted as learned once its guess is both locked and correct. Because the learner cannot know whether a guess is correct, active target selection in this variant prefers words whose guesses the learner has not yet locked in--information the learner actually has--rather than words it has not yet learned correctly.

We predicted this manipulation would, if anything, widen rather than narrow the active-learning advantage. Active selection, by preferentially returning to words whose guesses are not yet locked in, incidentally functions as a rehearsal schedule: it revisits a tentative guess again before a moderate forgetting hazard could plausibly erase it. Passive sampling has no such property--under Zipfian word frequencies, a rare word may go thousands of episodes between successive exposures, giving a tentative guess ample time to decay before it can be confirmed.

We calibrated the tested half-lives against the distribution of passive revisit gaps at $C=10$, $a=1$: the rarest word's expected gap between passive exposures is $\sim$7,485 episodes, and 97\% of words exceed a 200-episode gap, so we tested four half-lives spanning well below and above this range, $t_{1/2} \in \{20{,}000, 5{,}000, 1{,}000, 200\}$ episodes (equivalently $p \in \{0.000035, 0.000139, 0.000693, 0.003460\}$), with 30 replications per cell and a cap of two million episodes per run. Because some runs did not reach every checkpoint within this cap, we report medians, counting a run that never reached a checkpoint as slower than the cap.

The prediction was confirmed, and the effect was large and graded (Table~\ref{tab-decay}, Figure~\ref{fig-decay}). Requiring confirmation already slows both learners without any forgetting (compare the unconfirmed guess-test learner's 38,037 passive and 7,747 active episodes; Robustness across Zipf exponent and learning mechanism). Forgetting then slows active selection only modestly--its median episodes to 99\% roughly double between no forgetting and the shortest half-life tested--whereas passive sampling slows by more than an order of magnitude already at $t_{1/2}=1{,}000$, and at $t_{1/2}=200$ no passive run reached even 90\% of the vocabulary within two million episodes. The active/passive speedup at 99\% grows correspondingly, from 4.4x without forgetting to 56x at $t_{1/2}=1{,}000$ and more than 70x at $t_{1/2}=200$, and the same widening appears at every completion checkpoint (50\%, 90\%, and 99\%; see ``How much of the vocabulary matters,'' above). The gradient clarifies the mechanism: what matters for passive learning is whether the forgetting half-life is short relative to how long a tentative guess must wait to be rehearsed, which under Zipfian sampling depends on how rare the word is. Active selection keeps its own revisit gaps short for whatever it has not yet locked in, regardless of that word's rarity, so forgetting costs it comparatively little. The pattern does not depend on our choice of two confirmations: requiring three instead produced the same, if anything stronger, result (99\% speedup of 5.1x without forgetting, 17.6x at $t_{1/2}=5{,}000$, and more than 100x at $t_{1/2}=1{,}000$, where no passive run reached 99\% within the cap; at $t_{1/2}=200$ no passive run reached even 50\%, while active runs reached 99\% in a median of 48,700 episodes).

One qualification emerged. In a small minority of active runs with two-confirmation locking (2--4 of 30 per condition, including without forgetting; none with three confirmations, where a wrong guess is less likely to be confirmed by chance), learning stalled just short of 99\%: a few words had locked in a \textit{wrong} guess that happened to be confirmed by chance, and in doing so held on to the true referents of other, still-unlearned words. Because an active learner never revisits words it believes it already knows, these errors were never corrected. The medians in Table~\ref{tab-decay} are unaffected, since fewer than half the runs stalled, but the stall is a genuine limitation of a purely active policy: a learner that never re-checks its confident beliefs cannot recover from being confidently wrong, and some admixture of passively-encountered input--which real learners always have--would presumably be needed to repair such errors.
...
\begin{table}[h]
\centering
\caption{\label{tab-decay} Median episodes for the guess-test learner (guess locks after two consecutive confirmations; unlocked guesses decay if unrehearsed) to reach 50\%, 90\%, and 99\% of a Zipfian ($a=1$), $M=1{,}000$-word vocabulary, $C=10$, by forgetting half-life $t_{1/2}$ (episodes). ``$>$2M'': no run reached the checkpoint within the two-million-episode cap. Speedup is the ratio of passive to active medians.}
\small
\begin{tabular}{lrrrrrrr}
\hline
 & \multicolumn{3}{c}{Active} & \multicolumn{3}{c}{Passive} & Speedup \\
$t_{1/2}$ & 50\% & 90\% & 99\% & 50\% & 90\% & 99\% & at 99\% \\
\hline
No forgetting & 7{,}517 & 10{,}368 & 12{,}758 & 14{,}851 & 34{,}382 & 55{,}814 & 4.4x \\
20{,}000 & 7{,}589 & 10{,}670 & 13{,}700 & 16{,}142 & 39{,}802 & 74{,}310 & 5.4x \\
5{,}000 & 7{,}776 & 10{,}012 & 11{,}303 & 19{,}803 & 63{,}236 & 139{,}282 & 12.3x \\
1{,}000 & 10{,}310 & 13{,}136 & 14{,}510 & 49{,}325 & 297{,}240 & 815{,}432 & 56.2x \\
200 & 21{,}982 & 26{,}269 & 27{,}059 & 412{,}969 & $>$2M & $>$2M & $>$70x \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=\linewidth]{decay_lock_sweep.pdf}
\caption{\label{fig-decay}
Median episodes (log scale; bars = interquartile range) for the guess-test learner to reach 50\%, 90\%, and 99\% vocabulary completion, active vs.\ passive target selection, at $C=10$, $a=1$, across five forgetting conditions (no forgetting, and half-lives of 20,000, 5,000, 1,000, and 200 episodes for a guess not yet locked in by two consecutive confirmations). Open triangles mark cells in which the median run did not reach the checkpoint within the two-million-episode cap. Forgetting slows active selection modestly, but slows passive sampling by orders of magnitude as the half-life shortens, widening the active/passive gap at every checkpoint.}
\end{center}
\end{figure}
...
Analyzing an empirical dataset of labeled referents in a set of natural scenes, we found that the expected learning time of an idealized active learner--one with full advance knowledge of how referents co-occur with scenes, who still fast-maps each word--is over two hundred times (235x) more efficient than a passive learner sampling scenes uniformly at random. The same data show that real scene statistics are far harder to learn from than an idealized Zipfian distribution of the same size (by a factor of about 3,990 under passive sampling), and that even the optimal active learner remains slower than a passive learner facing that idealized distribution: active selection recovers much, but not all, of what real-world skew costs.
Three further analyses show that this large speedup should nonetheless not be treated as a single, representative estimate, because the advantage depends on the structure of the environment, on the learning mechanism, and on how large the local context is relative to the vocabulary.
First, in a synthetic setting, an active learner who must estimate the co-occurrence structure $P$ online from experience, rather than know it in advance, performed as well as the optimal learner who knows it (see the Active learning section), suggesting that learning $P$ need not be costly. In that setting, however, even the optimal learner needed only about half as many samples as a passive one: how much active selection can gain depends heavily on the structure of $P$, from about 2x in our synthetic matrix to 6.7x (Flickr8k) and 235x (SUN) in real data.
Second, a learner who cannot fast-map at all, but must instead accumulate evidence for word-referent mappings across multiple exposures, shows an advantage that depends heavily on context size: for an eliminative learner, active target selection alone yields a 19-fold advantage under a Zipfian distribution when contexts are small relative to the vocabulary ($C/M=0.01$), and 29-fold when combined with active control over the accompanying distractors. When contexts are large enough to reach \cite{Vogt:2012}'s reported tractability boundary ($C/M=0.1$), learning with randomly-composed contexts never completes at all, whatever the target selection; only control over the distractors restores it, after which active target selection adds a further 28-fold advantage (see the Incremental active learning section).
Third, this advantage is not a fixed property of ``incremental learning'' in general: a distinct propose-and-revise (guess-test) mechanism shows the same qualitative pattern but a substantially more modest magnitude (roughly 2--5x across conditions rather than 19--29x), and, because it never needs to rule out every competing referent, it keeps learning at $C/M=0.1$ where full elimination fails.
Taken together, these estimates range from about 2x to 235x, and, at $C/M=0.1$, from learning that never completes to learning that does, depending on the structure of the environment, whether learning requires one exposure or several, which specific learning mechanism is assumed, and how large the local context is relative to the vocabulary.
...
On one hand, infants' attention is selective in ways that depend on what they already know: they preferentially attend to stimuli of intermediate, rather than minimal or maximal, predictability or complexity \citep[the ``Goldilocks effect'';][]{Kidd:2012}. That preference differs from our active learner's, which targets the least-known word, and the companion paper (in preparation) tests a Goldilocks-style policy directly: it matches least-known targeting in most settings and improves on it in some. Self-directed, environment-adapted information search is documented in children from around age three onward \citep{Ruggeri:2022}, well within the age range relevant to our pre-literacy scope (see the Introduction).
...
Fourth, every learner in this paper retains a tentative guess indefinitely once made, with no possibility of forgetting it if it goes unrehearsed--a strong idealization a skeptical reader might expect to be doing unacknowledged work in active selection's favor, since an active learner's advantage could partly reflect its access to a kind of perfect memory no real learner has. We tested this directly by adding a minimal forgetting mechanism to the guess-test learner (``Does the advantage survive forgetting?'', above) and found the opposite of what that worry would predict: once a guess must be confirmed before it sticks, forgetting roughly doubled active selection's episode count at the most severe rate tested, but slowed passive sampling by more than an order of magnitude--to the point that passive learners never reached 90\% of the vocabulary within our simulation budget--because active selection's habit of preferentially returning to what remains unknown incidentally functions as a rehearsal schedule that protects it from exactly the kind of decay a passive learner, with its much longer and more variable revisit gaps under Zipfian sampling, cannot avoid.

Fifth, every learner in this paper narrows a word's candidate referents using only that word's own history of exposures, with no information sharing across words beyond the frequency distributions they share--an independence assumption raised as a central concern about this modeling tradition more broadly. \cite{Reisenauer:2013} address this directly for an elimination-based learner closely related to ours: adding a mutual exclusivity constraint, whereby confirming one word's meaning immediately excludes that referent as a candidate for every other still-unlearned word, induces a dynamical phase transition--below a critical context size, learning collapses to the fastest it can possibly be (limited only by the time to encounter each word at least once), and even above that threshold it remains dramatically faster than when words are learned independently. We did not implement this mechanism in our main simulations, so the numbers reported in the body of the paper--especially the eliminative model's outright failure to learn with random contexts at $C=100$ (Table~\ref{tab-incremental})--should be read as an upper bound on how slow elimination-based learning really is under referential uncertainty, rather than a tight estimate. We report a direct test of cross-word mutual exclusivity under our own Zipfian word and referent distributions, separately from the main results, in Appendix~\ref{app-mutex}: it delivers the predicted rescue, turning that failure into learning within under a million episodes, but for passive and active target selection alike, so that active selection gains little from it at this context size--for a specific, verified reason having to do with how learning begins under the mechanism's cascade dynamics.

This independence assumption also has direct empirical, not just mathematical, consequences worth confronting directly. \cite{Hendrickson:2019} tested human cross-situational word learning under Zipfian vs.\ uniform distributions and found, across three experiments with adult learners, that Zipfian distributions are not merely tolerable but actively \textit{easier} to learn from--the opposite of what every mathematical analysis in this tradition, including our own, predicts. Their proposed mechanism is precisely the one we test in Appendix~\ref{app-mutex}: learning the high-frequency items quickly lets a mutual-exclusivity-style disambiguation process narrow down the remaining low-frequency items faster than a uniform environment, with its uniformly slow start, ever could. A follow-up study extends this finding from adults to children learning novel word-referent mappings \citep{Wolters:2024}, and a control condition in \cite{Hendrickson:2019} in which items were presented one at a time rather than cross-situationally eliminated the Zipfian advantage entirely, confirming that it depends specifically on the cross-situational disambiguation process our own mutual exclusivity extension targets. Our own result is more modest than theirs: cross-word mutual exclusivity substantially narrows, but for passive sampling does not fully erase, the Zipfian penalty relative to a matched uniform distribution (from 13x to 4.8x at $C=10$; Appendix~\ref{app-mutex}). This may reflect differences in scale (60 words across a handful of screens vs.\ our $M=1{,}000$-word simulations), in how strongly human memory weights recent or frequent items beyond what our idealized, error-free elimination captures, or in some other idealization we do not model. Reconciling the exact magnitude is a task for future work, but the qualitative direction of both results agrees: mutual exclusivity, not assumed away by learning words independently, is doing real work in making Zipfian word learning tractable, and may be doing considerably more of that work in real human learners than our own simulations, absent this mechanism, would suggest.

One further modeling choice affects the size of the familiar-context benefit we report. Familiar-context selection weights each candidate distractor by how few words still hold it as a possible meaning. In the eliminative learner, a word that has never yet been heard has ruled nothing out, and so counts as holding every referent as a possible meaning; while many words are still unheard, they add the same amount to every referent's count and blur exactly the differences between referents that familiar-context selection relies on. A learner that judges familiarity only from words it has actually encountered avoids this dilution. At $C=100$, $a=1$, such a learner reaches 99\% of the vocabulary in about half the episodes under active target selection (7,803 rather than 15,770, 30 replications each), while passive sampling, whose bottleneck lies in how rarely its rare words are targeted, is essentially unaffected (451,567 rather than 466,339). The 28-fold advantage of active target selection under familiar context at $C=100$ (Table~\ref{tab-incremental}) would therefore roughly double under this alternative bookkeeping, so our familiar-context results are, if anything, conservative. The dilution also makes the benefit of familiar-context selection sensitive to which words a learner reaches early: policies that concentrate attention on frequent words leave many words unheard for longer, and so delay the point at which familiar-context selection starts to help. We examine this in the companion paper.

Three further questions are outside this paper's scope because they concern how active selection's benefit is deployed rather than whether it exists, and we develop them in a companion paper (in preparation) instead: whether a learner should linger on the current situation to extract more information from it or seek a new one, when doing so carries a real cost; whether the developmental trajectory by which real learners move from largely passive to increasingly self-directed matters in its own right, independent of the time-averaged degree of autonomy it implies; and how much of active selection's benefit survives when a learner exercises choice only some of the time, or only among the objects at hand.
...
Write $T_{+} = f_{W, \epsilon}( \min p )$, which gives a reasonable estimate of the upper bound of the root $T$ of (\ref{eqA-M}).
...
= 
 \frac{ 
p_{\min} 
\left( \frac{ \partial H_{W, a} }{ \partial a}/ H_{W, a} + \log W \right)
...
\]
where $e_{1} := (1, 0, \ldots, 0 )^{T} \in \mathbb{R}^{M}$.
Then for $0 < n \le N$, 
define
\begin{eqnarray}
\nonumber
...
This gradient is already quite suggestive: the gradient of $g$ in the limit is a function of both $q$ and the minimum index $j = \argmin_{i}P_{i}q$. Namely, the gradient reflects the tentative hyper-plane $P_{\min}q = C$. Taking small steps along this gradient (with a Lagrange multiplier for the constraint $q \in \Simplex{M}$), as in gradient ascent, increases $f(q)$ toward its maximum.
...
$C=100$, Passive & $>5 \times 10^8$ & 771{,}125 & $>$640x \\
$C=100$, Active & $>5 \times 10^8$ & 686{,}349 & $>$720x \\
\hline
\end{tabular}
\end{table}

At $C=10$, mutual exclusivity helps both target-selection policies, roughly 3--4-fold. At $C=100$--the case motivating this appendix, since without it the eliminative learner never completes at all--mutual exclusivity delivers the rescue \cite{Reisenauer:2013} predict: both passive and active learners now reach 99\% of the vocabulary, in well under a million episodes on average. The trajectory is unusual, however. Essentially the whole vocabulary is learned in a single cascade: in 396 of 400 replications, the 50\%, 90\%, and 99\% checkpoints were all reached within 10 episodes of one another (and in the rest within about 16,000, still negligible next to the total), after a long initial period in which almost nothing was learned. And active target selection, the most reliable source of speedup everywhere else in this paper, confers little benefit here: 686,349 versus 771,125 episodes (medians 441,212 and 572,126), a difference within sampling error given the heavy-tailed learning times ($z \approx 1.3$).

Mutual exclusivity also narrows, without erasing, the penalty for a Zipfian rather than a uniform distribution. Under a uniform distribution, where the eliminative learner has no trouble at either context size, mutual exclusivity helps only modestly (passive: 6,907 to 4,858 episodes at $C=10$, and 9,906 to 6,667 at $C=100$; 30 replications each). For passive sampling, the Zipfian-to-uniform ratio therefore falls from 13x to 4.8x at $C=10$, and at $C=100$ from never completing to 116x.

Both features of the $C=100$ trajectory have the same explanation. Under mutual exclusivity, nothing can cascade until at least one word has been learned by ordinary elimination, which requires ruling out every competitor for that word--the same requirement that makes independent elimination fail at this context size, and which is least severe for the most frequent word, since its own referent is the one competitor that is otherwise hardest to rule out. Once that first word is learned, its referent is removed from every other word's candidate set, which unblocks the next, and so on through the vocabulary. The wait for that first learning event dominates the total time. And during that wait, active target selection is inert: its advantage throughout this paper comes from never wasting an episode on an already-known word (formally, \texttt{choose\_target}'s active branch restricts its candidate pool to not-yet-known words before sampling), but while no word is yet known, that restriction is a no-op. We confirmed directly (holding the random seed fixed) that active and passive target selection produce identical sequences of targets for as long as no word is yet known, and diverge only from the first moment any word becomes known--by which point, at $C=100$, the cascade has usually already done nearly all the work.

We see two useful takeaways. First, \cite{Reisenauer:2013}'s rescue of elimination-based learning at large $C$ is real and dramatic in our own Zipfian word-and-referent setting, not only in their resampled-Zipf confounder construction: it turns a learner that never finishes into one that finishes within a million episodes, reinforcing the Discussion's point that this paper's $C=100$ random-context results overstate the true difficulty of learning under referential uncertainty. Second, mutual exclusivity and active target selection, both independently beneficial mechanisms elsewhere in this paper, do not simply compound at large $C$: the source of active selection's advantage (avoiding wasted attention on known words) and the source of mutual exclusivity's advantage (a cascade triggered by the first learned word) barely overlap. Working out the general conditions under which two otherwise-beneficial mechanisms fail to compose--already noted for guess-test's active-target/familiar-context combination in the main text--seems a genuinely open question for this literature, not a peculiarity of any one mechanism pairing.
