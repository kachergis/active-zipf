...
We can also test the specific concern raised above--that idiosyncratic, single-child words inflate the estimate--more directly, by recomputing $T_{+}$ on the same CHILDES corpus after removing words that occur only once (hapax legomena), the words most likely to be exactly this kind of artifact. Doing so shrinks the vocabulary from 51,446 to 27,673 words and raises the minimal word probability from $1.089\times10^{-7}$ to $2.616\times10^{-7}$, giving $T_{+} = f_{27673,0.01}(2.616\times10^{-7}) = 5.67\times10^{7}$--about 2.5 times smaller than the original estimate, but still the same order of magnitude, and still far larger than the number of samples a child could plausibly encounter. Removing all words rarer than 11 occurrences in the corpus (a considerably more aggressive filter, leaving 10,179 words) shrinks $T_{+}$ further, to $9.52\times10^{6}$, which is close to the analytical, idealized-Zipfian estimate from earlier in this section. Even under this more aggressive filtering--which almost certainly removes some genuinely rare but real words along with any idiosyncratic ones--the qualitative conclusion that passive fast-mapping from a realistic word frequency distribution is slow is unchanged; only its precise magnitude is sensitive to exactly how the rarest words in a finite corpus are treated.
...
Having access to the underlying data lets us answer directly, rather than by conjecture, a question raised in an earlier version of this analysis: is the passive marginal realized under uniform scene selection more or less severe than the idealized Zipfian distributions analyzed in Section~\ref{sec-fastmap}?
...
Two further checks address whether these conclusions are specific to $a=1$ or to the eliminative mechanism. First, varying the Zipf exponent at $C=10$ (Figure~\ref{fig-exponent}) shows the same dose-response pattern found analytically in Section~\ref{sec-fastmap} (Figure~\ref{fig-estimate}): passive learning slows super-exponentially as $a$ increases (eliminative passive/random: 6,676 episodes at $a=0$, 10,795 at $a=0.5$, 89,454 at $a=1$), and active selection's advantage grows accordingly.
...
Second, a learner who cannot fast-map at all, but must instead accumulate evidence for word-referent mappings across multiple exposures, shows an advantage that depends heavily on context size: for an eliminative learner, active target selection alone yields a 16--19-fold advantage under a Zipfian distribution, and combining it with active control over the accompanying distractors yields a 29-fold advantage when contexts are small relative to the vocabulary ($C/M=0.01$), but a more than 1,000-fold advantage when contexts are large enough to approach \cite{Vogt:2012}'s reported tractability boundary ($C/M=0.1$; Section~\ref{sec-incremental-sim}).
...
What remains stable across this range is the direction of the effect, not its exact size: some form of active control over learning situations reliably speeds up vocabulary acquisition under Zipfian word and referent distributions, and does so far more than under a uniform distribution, where active selection helps only modestly (Section~\ref{sec-incremental-sim}).
