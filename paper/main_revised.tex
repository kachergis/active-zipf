...
The analyses above--both the fast-mapping bound of Section~\ref{sec-fastmap} and the idealized and online active learners of Section~\ref{sec-active}--all assume that a word is learned the moment it co-occurs with its target referent. This lets us derive clean analytical results, but it sidesteps a concern raised about this modeling tradition more broadly: that conclusions about learning speed may reflect the {\it sampling} process (how often the right word-referent pair happens to co-occur) rather than anything about {\it learning} itself, and that treating each word as learned independently and instantaneously is a strong idealization. To test whether our qualitative conclusions--that passive sampling from a Zipfian distribution is implausibly slow, and that active learners can close much of this gap--survive when both of these idealizations are relaxed, we simulated a learner who accumulates evidence across multiple exposures rather than fast-mapping.
...
More importantly than the exact target size is whether the child must learn a specific, enumerated set of $W$ words (a harder problem, since the rarest words in any sample from a Zipfian distribution are, by construction, very rare) or merely {\it some} sufficiently large subset of the words available in the environment (an easier problem, since the most frequent fraction of a Zipfian distribution is itself Zipfian, and thus reachable in substantially fewer samples than the full tail).
