...
The prediction was confirmed, and the effect was substantial (Figure~\ref{fig-decay}). At $C=10$, $a=1$, introducing forgetting at any of three tested half-lives (200, 50, or 10 episodes, i.e.\ $p \in \{0.0035, 0.014, 0.067\}$) left the active condition essentially unchanged--6,797--7,266 episodes to learn 99\% of the vocabulary, indistinguishable from the no-decay baseline of 7,656--but nearly doubled the passive condition, from 38,774 to 68,179--72,804 episodes. The active/passive speedup accordingly grew from 5.1x with no decay to 9.4--10.4x with any decay present, and this pattern held at every completion checkpoint we examined (50\%, 90\%, and 99\%; see ``How much of the vocabulary matters,'' above), not only at the most demanding one. Once any decay was present at all, its exact severity mattered comparatively little--68,179 episodes at a 200-episode half-life versus 72,804 at a 10-episode half-life, a difference small relative to the gap from having no decay at all--suggesting that what matters for passive learning is less the precise forgetting rate than whether tentative guesses can go unrehearsed for many episodes at all, which they routinely can under Zipfian passive sampling but rarely can under active selection.
...
Mean episodes ($\pm$ 1 SE, log scale) for the guess-test learner to reach 50\%, 90\%, and 99\% vocabulary completion, active vs.\ passive target selection, at $C=10$, $a=1$, across four forgetting conditions (no decay, and forgetting half-lives of 200, 50, and 10 episodes for an unconfirmed guess). Forgetting leaves active selection essentially unaffected while substantially slowing passive sampling, widening rather than narrowing the active/passive gap.}
\end{center}
\end{figure}
